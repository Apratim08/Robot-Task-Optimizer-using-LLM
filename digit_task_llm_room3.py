...
async def main(host_address, task):
    ...
    # package properties
        ...
        await api.send(msgs.AddLandmarks(
            map_name='table4-map',
            landmarks=[
                msgs.Landmark(id=31, pose=msgs.Pose(axyzd=[0, 0.375, -0.6, 0])),
                msgs.Landmark(id=32, pose=msgs.Pose(axyzd=[0, 0.375, 0.6, 0]))]
        ))

        # # Add the fourth table and its keep out zone
        await api.get_response(msgs.AddObject(
            attributes={
                'name': 'table-4',
                'box-geometry': table4['geometry'],
                'polygon': set_box_polygon(table4['geometry']),
                'keep-out': True
            },
            transform={'xyz': [0, 0, 0]},
            relative_to={'map-name': 'table4-map'}
        ))
        ...
        # AprilTag on box
        await api.get_response(msgs.AddObject(
            attributes={'april-tag-id': 3},
            transform={'xy': [package['geometry'][0]/2, 0]},
            relative_to={'owned-object-name': 'coffee'}
        ))
        await api.get_response(msgs.AddObject(
            attributes={'april-tag-id': 4},
        ...
        await api.get_response(msgs.AddObject(
            ...
            transform={'xy': [package['geometry'][0]/2, 0]},
            relative_to={'owned-object-name': 'green-box'}
        ))
        await api.get_response(msgs.AddObject(
        ...
        await api.get_response(msgs.AddObject(
            attributes={'april-tag-id': 17},
            transform={'xy': [package['geometry'][0]/2, 0]},
            relative_to={'owned-object-name': 'blue-box'}
        ))
        await api.get_response(msgs.AddObject(
            attributes={'april-tag-id': 18},
            transform={'axyd': [180, -package['geometry'][0]/2, 0]},
            relative_to={'owned-object-name': 'blue-box'}
        ))

        await api.get_response(msgs.AddObject(
            attributes={'april-tag-id': 27},
            transform={'xy': [package['geometry'][0]/2, 0]},
            relative_to={'owned-object-name': 'red-box'}
        ))
        await api.get_response(msgs.AddObject(
            attributes={'april-tag-id': 28},
            transform={'axyd': [180, -package['geometry'][0]/2, 0]},
            relative_to={'owned-object-name': 'red-box'}
        ))
        ...
        await api.wait_action(msgs.ActionGoto(
        target=msgs.Pose(axyd=[180, 1.25, -7]),
        reference_frame=msgs.ObjectSelector(map_name='table1-map')
        ))
        ...
        # mob.leg_length = 1
        # await api.send(msgs.ActionPick(
        #     {'owned-object-name': 'coffee'},mobility_parameters=mob
        # ))
        ...
        #         pose=msgs.Pose(xyz=[package['geometry'][0]/2, -0.5, package['geometry'][2]/2+0.1]),
        #         reference_frame=msgs.ObjectSelector(map_name='table3-map')))
        #     await asyncio.sleep(3)
        # except agility.exceptions.ActionFailed:
        #     print('action failed')
        ...
        #     await api.wait_action(msgs.ActionPlace(
        #         position_tolerance = 0.5,
        #         pose=msgs.Pose(xyz=[package['geometry'][0]/2, 0, package['geometry'][2]/2+0.1]),
        #         reference_frame=msgs.ObjectSelector(map_name='table2-map')))
        #     await asyncio.sleep(3)
        # except agility.exceptions.ActionFailed:
        #     print('action failed')
        ...
        # mob.leg_length = 1
        # await api.send(msgs.ActionPick(
        #     {'owned-object-name': 'green-box'},mobility_parameters=mob
        # ))
        # await asyncio.sleep(10)

        # await api.wait_action(msgs.ActionGoto(
        # target=msgs.Pose(axyd=[180, 1.5, 0]),
        # reference_frame=msgs.ObjectSelector(map_name='table4-map')
        # ))
        ...
        # mob.leg_length = 1
        # await api.send(msgs.ActionPick(
        #     {'owned-object-name': 'blue-box'},mobility_parameters=mob
        # ))
        # await asyncio.sleep(10)
        ...
        #     await api.wait_action(msgs.ActionPlace(
        #         position_tolerance = 0.5,
        #         pose=msgs.Pose(xyz=[package['geometry'][0], 0, package['geometry'][2]/2+0.1]),
        #         reference_frame=msgs.ObjectSelector(map_name='table1-map')))
        #     await asyncio.sleep(3)
        # except agility.exceptions.ActionFailed:
        #     print('action failed')
